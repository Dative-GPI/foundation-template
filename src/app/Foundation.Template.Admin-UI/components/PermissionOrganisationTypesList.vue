...
import { usePermissions, usePermissionOrganisationTypes } from "../composables";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // const { update, updating } = usePermissionOrganisationTypes();
    ...
    const fetchPermissionOrganisationTypes = async () => {
      getPermissionOrganisationTypes({ search: search.value }).then(() => {});
    };
    ...
    const updatedAll = (ev: Event) => {
      if (enabledAll.value) {
        permissionIds.value = filteredPermissionOrganisationTypes.value.map((p) => p.id);
      } else {
        permissionIds.value = [];
      }
    ...
    const updatePermission = (permissionId: string) => {
      ...
      // update(props.organisationTypeId, { permissionIds: permissionIds.value });
    };

    return {
      permissionOrganisationTypes,
      search,
      permissionIds,
      // updating,
      enabledAll,
      element,
      filteredPermissionApplications: filteredPermissionOrganisationTypes,
      updatePermissionIds,
      updatePermission,
      hasPermission,
      fetchPermissionOrganisationTypes,
